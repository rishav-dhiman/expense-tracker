  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 relative flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-[420px] bg-white border border-gray-200 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 md:p-10 flex flex-col items-center">
        <h1 className="text-[28px] font-[700] tracking-[-0.03em] mb-1.5 text-black">Create an account</h1>
        ...
        <form onSubmit={handleSubmit} noValidate className="w-full flex flex-col gap-4">
          <div className="flex flex-col w-full text-left">
            <label className="text-[13px] font-[600] text-gray-800 mb-1.5 flex justify-between">Full Name
               {fieldErrors.name && <span className="text-red-500 font-normal">{fieldErrors.name}</span>}
            </label>
            <input 
              type="text" 
              value={name} 
              ...
              className={`w-full px-3 py-2.5 text-[14px] bg-white border ${fieldErrors.name ? 'border-red-400 ring-1 ring-red-400' : 'border-gray-200'} rounded-md focus:outline-none focus:border-gray-400 focus:ring-1 focus:ring-gray-400 transition-all font-medium placeholder-gray-400`}
            />
          </div>
        ...
        <p className="text-center mt-6 text-[13px] text-gray-500 font-medium">
          Already have an account? <Link to="/login" className="text-gray-900 font-[700] hover:underline">Log in</Link>
        </p>
